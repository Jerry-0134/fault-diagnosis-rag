...
# 立即执行初始化
init_ok, case_count = init_knowledge_base()

# Session state must exist before sidebar image actions can append messages.
...
with st.sidebar:
    ...
    if init_ok:
        st.success(f"✅ 知识库已就绪 ({case_count} 条)")
    else:
        st.error("❌ 知识库初始化失败")
        ...
        if st.button("🔍 分析图片并诊断", key="analyze_btn", type="primary"):
            with st.spinner("正在分析图片..."):
                ...
                with st.spinner("正在生成诊断建议..."):
                    answer, hits = query(image_query, top_k=TOP_K)
        ...
        if "sources" in msg and msg["sources"]:
            with st.expander("📋 查看参考案例"):
                for src in msg["sources"]:
                    ...
                    st.caption(f"**{title}** (综合分: {src.get('score', 0):.3f})")
...
if prompt := st.chat_input("请输入故障现象或问题..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("正在检索相关案例并生成回答..."):
            ...
            answer, hits = query(prompt, top_k=TOP_K)
            ...
            if hits:
                with st.expander("📚 查看参考案例"):
                    for hit in hits:
                        ...
                        st.caption(f"**{title}** (综合分: {hit.get('score', 0):.3f})")
